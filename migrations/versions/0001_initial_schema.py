...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("device_id", sa.String(length=128), sa.ForeignKey("devices.device_id"),
                  nullable=False),
        ...
        sa.UniqueConstraint(
            "device_id", "observed_at", "latitude_e7", "longitude_e7",
            name="uq_observation_identity",
        ),
    )
    op.create_index("ix_obs_device_observed", "location_observations",
                    ["device_id", "observed_at"])
    op.create_index("ix_obs_observed", "location_observations", ["observed_at"])
    op.create_index("ix_obs_device_fetched", "location_observations",
                    ["device_id", "first_fetched_at"])
    ...
    op.create_table(
        ...
        sa.Column("observations_returned", sa.Integer(), nullable=False,
